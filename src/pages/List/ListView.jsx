    ...
    <div className="h-screen bg-[#1c1c1c] text-white container-sm px-5 py-5 md:px-40 lg:px-52">
      ...
      {posts.length === 0 ? (
        ...
        posts.map((post) => (
          <div
            key={post.id}
            className="bg-black w-full my-5 p-5 rounded-md shadow shadow-gray-400 cursor-pointer transition hover:shadow-pink-500"
          >
      ...
      {/* bir kullanıcı seçilince ekrana onun gönderilerini bastığımız pencereyi aç */}
      {selectedUser && (
        <PopupController
          user={selectedUser}
